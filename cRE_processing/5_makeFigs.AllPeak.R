...
#===========================================================================
## load data
data1 <- read.table('CountMatrix/H3K27ac.RPM.qq.zscore.txt', header=T, row.names=1)
valMat <- data1
...
## Heatmap
pdf('Plots/H3K27ac.allPeak.corHeatmap.pdf')
...
#    geom_text(aes(x=df$PC2 ,y=df$PC1, label=rownames(df)), size=1.2) +
...
pdf("Plots/H3K27ac.allPeak.PCA.pdf", height=3, width=6, pointsize=3)
plot(g1)
dev.off()
