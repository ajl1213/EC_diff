...
def loadSigPeaks():
    PeakDict={}
    input1=open(PeakDIR+'/'+cellType+'.bed','r')
...
def genRandPeak():
    ...
    output1=open(DIR+'/RandPeakPermute/'+gwasID+'.'+cellType+'.'+nCopy+'.RandPeak.bed','w')
...
def permuteRandPeaks():
    output1=open(DIR+'/RandPeakPermute/'+gwasID+'.'+cellType+'.'+nCopy+'.RandPeakPermuteRecord.txt','w')
	...
	##
	runScript='intersectBed -a '+DIR+'/RandPeakPermute/'+gwasID+'.'+cellType+'.'+nCopy+'.RandPeak.bed -b '+LdSnpDIR+'/'+gwasID+'.bed -wa -wb | cut -f1,2,3 | sort -u | wc -l > '+DIR+'/RandPeakPermute/'+gwasID+'.'+cellType+'.'+nCopy+'.RandPeak.intersect.bed'
	#print runScript
	os.system(runScript)

	##
	input1=open(DIR+'/RandPeakPermute/'+gwasID+'.'+cellType+'.'+nCopy+'.RandPeak.intersect.bed', 'r')
	all_input1=input1.readlines()
